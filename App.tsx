...
const createProxyUrl = async (sourceUrl: string): Promise<string> => {
  ...
  return new Promise((resolve) => {
    const img = new Image();
    img.onload = () => {
      const scale = Math.min(1, PROXY_SIZE / Math.max(img.width, img.height));
      if (scale >= 1) { resolve(sourceUrl); return; }
      const canvas = document.createElement('canvas');
...
const App: React.FC = () => {
  ...
  const [isBeta, setIsBeta] = useState(() => {
    if (typeof window !== 'undefined') return localStorage.getItem('fstop64_beta_mode') === 'true';
    return false;
  });
  ...
  if (isBeta) return <BetaApp onToggleBeta={toggleBeta} />;
  ...
  const handleUpdateParams = (newParams: EditParams) => {
    ...
    if (activePhoto && activeImage && newParams.crop.rotation !== activePhoto.params.crop.rotation) {
      ...
      newParams.crop.top = safeCrop.top; newParams.crop.bottom = safeCrop.bottom;
      newParams.crop.left = safeCrop.left; newParams.crop.right = safeCrop.right;
    }
  ...
  const triggerExportFlow = (type: 'single' | 'batch') => {
    ...
    setExportTarget(type); setModalType('export_options');
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-[#121212] text-[#d4d4d4] overflow-hidden">
      ...
      <input type="file" ref={fileInputRef} className="hidden" accept="image/*,.arw,.cr2,.cr3,.nef,.dng,.orf,.raf,.rw2,.pef,.srw,.tif,.tiff" multiple onChange={handleFileChange} />
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden relative">
        <div className="flex-1 flex flex-col bg-[#1a1a1a] min-w-0 overflow-hidden h-1/2 md:h-auto">
          ...
          <Filmstrip photos={photos} activePhotoId={activePhotoId} onSelect={setActivePhotoId} onAdd={() => fileInputRef.current?.click()} onExportSpecific={() => triggerExportFlow('single')} />
        </div>
        <aside className="w-full md:w-80 h-1/2 md:h-full bg-[#1e1e1e] border-t md:border-t-0 md:border-l border-zinc-800 flex flex-col z-10 shadow-2xl flex-shrink-0">
          <Sidebar params={activePhoto?.params || DEFAULT_PARAMS} onChange={handleUpdateParams} presets={presets} onSavePreset={(name) => activePhoto && savePreset(name || `Preset ${presets.length + 1}`, activePhoto.params)} onApplyPreset={(p) => handleUpdateParams({ ...p.params })} onDeletePreset={deletePreset} editedPhotos={editedPhotos} onBatchExportEdited={() => triggerExportFlow('batch')} onSelectPhoto={setActivePhotoId} onDismissPhoto={(id) => setPhotos(prev => prev.map(p => p.id === id ? { ...p, hiddenFromEdited: true } : p))} onUndoDismiss={() => { }} hasLastDismissed={false} isCropMode={isCropMode} onToggleCropMode={() => setIsCropMode(!isCropMode)} />
        </aside>
      </div>
    </div>
  );
